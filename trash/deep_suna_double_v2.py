...
"""
Serial communication program for construction machine.
...
class DeepSuna(AI_Player):
    ...
    def __init__(self, checkpoint_path="deep_suna_networks", playback_mode=False, verbose_logging=False):
        ...
        # self.saver.restore(self._session, "deep_suna_networks/relearning_model24100000")
    ...
    def get_keys_pressed(self, screen_array, reward, terminal, turn):
        ...
        if self._time % self.TARGET_NETWORK_UPDATE_FREQ == 0 and self._time > 1:
            convert1 = [self.target_kernel1, self.target_kernel2, self.target_kernel3, self.target_biases1,
                        self.target_biases2, self.target_biases3, self.target_biases4, self.target_weights4,
                        self.target_weights6]
            convert2 = [self.kernel1, self.kernel2, self.kernel3, self.biases1,
                        self.biases2, self.biases3, self.biases4, self.weights4,
                        self.weights6]
            for i in range(0, 9):
                convert3 = self._session.run(convert2[i])
                change = convert2[i].assign(convert1[i])
                self._session.run(fetches=change)
                change_target = convert1[i].assign(convert3)
                self._session.run(fetches=change_target)
