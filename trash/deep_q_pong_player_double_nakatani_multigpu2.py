...
class DeepQPongPlayer(PongPlayer):
    ...
    MINI_BATCH_SIZE = 75  # size of mini batches
    ...
    _session = tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False))
    ...
    def __init__(self, checkpoint_path="deep_q_pong_networks", playback_mode=False, verbose_logging=False):
        ...
        for d in [0, 1]:
            ...
            with tf.device('/gpu:%d' % d):
                '''
                self._input_states[d] = (tf.placeholder("float",
                                                        [None, DeepQPongPlayer.RESIZED_SCREEN_X,
                                                         DeepQPongPlayer.RESIZED_SCREEN_Y,
                                                         DeepQPongPlayer.STATE_FRAMES]))
                '''
        ...
        for d in [0, 1]:
            with tf.device('/gpu:%d' % d):
                '''
                self._target_input_states[d] = (tf.placeholder("float",
                                                               [None, DeepQPongPlayer.RESIZED_SCREEN_X,
                                                                DeepQPongPlayer.RESIZED_SCREEN_Y,
                                                                DeepQPongPlayer.STATE_FRAMES]))
                '''
                target_hidden_convolutional_layer_1 = tf.nn.relu(
                    tf.nn.conv2d(self._target_input_states[d], self.target_convolution_weights_1, strides=[1, 2, 2, 1],
                                 padding="SAME") + self.target_convolution_bias_1)

                target_hidden_max_pooling_layer_1 = tf.nn.max_pool(target_hidden_convolutional_layer_1,
                                                                   ksize=[1, 2, 2, 1],
                                                                   strides=[1, 2, 2, 1], padding="SAME")

                target_hidden_convolutional_layer_2 = tf.nn.relu(
                    tf.nn.conv2d(target_hidden_max_pooling_layer_1, self.target_convolution_weights_2,
                                 strides=[1, 2, 2, 1],
                                 padding="SAME") + self.target_convolution_bias_2)

                target_hidden_max_pooling_layer_2 = tf.nn.max_pool(target_hidden_convolutional_layer_2,
                                                                   ksize=[1, 2, 2, 1],
                                                                   strides=[1, 2, 2, 1], padding="SAME")
                ...
                target_hidden_max_pooling_layer_3 = tf.nn.max_pool(target_hidden_convolutional_layer_3,
                                                                   ksize=[1, 2, 2, 1],
                                                                   strides=[1, 2, 2, 1], padding="SAME")
                ...
                self.target_output_layer[d] = tf.matmul(
                    target_final_hidden_activations,
        ...
        self.summary_writer = tf.train.SummaryWriter("/home/uchi/catkin_ws/src/PyGamePlayer/apple_game/na_logs",
                                                     self._session.graph)
        '''
