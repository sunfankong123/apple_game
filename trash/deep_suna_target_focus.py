...
class DeepSuna(AI_Player):
    ...
    def __init__(self, checkpoint_path="deep_suna_networks", playback_mode=False, verbose_logging=False):
        ...
        self._target = tf.placeholder("float", [None, self.ACTIONS_COUNT], name="target_Q")
        ...
        # self.readout_action = tf.reduce_sum(tf.mul(self.output_layer, self._action), reduction_indices=1)
        self.cost = tf.reduce_mean(tf.square(self._target - self.output_layer))
        ...
        self.fileinputdata = open("inputdata_middlefield_route1.txt", "r")
        # self.fileloss = open("lossdata.txt", "w")

        self.saver = tf.train.Saver()
        # self.saver.restore(self._session, "networks_pinkjuki_no_dropout_170205/model4000000")
        '''
    ...
    def _choose_next_action(self):
        ...
        if len(self._observations) < self.HUMAN_CONTROL:
            ...
            self.readout_t = self._session.run(self.output_layer, feed_dict={self._input_states: [self._last_state]})[0]
    ...
    def _train(self):
        ...
        agents_previous_q_action = self._session.run(self.output_layer,
                                            feed_dict={self._input_states: previous_states})
        action_previous = np.argmax(agents_previous_q_action)
