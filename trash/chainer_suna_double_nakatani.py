...
class DeepSuna(AI_Player):
    ...
    def __init__(self, checkpoint_path="deep_suna_networks", playback_mode=False, verbose_logging=False):
        ...
        self._last_state = np.zeros((self.chainer_dqn_class.STATE_FRAMES,
    ...
    def get_keys_pressed(self, screen_array, reward, terminal, turn):
        ...
        if self.first_frame is True:
            ...
            compute_state = cuda.to_gpu(cupy.asanyarray(self._last_state.reshape(1, self.chainer_dqn_class.STATE_FRAMES,
        ...
        compute_state = cuda.to_gpu(cupy.asanyarray(current_state.reshape(1, self.chainer_dqn_class.STATE_FRAMES,
                                                                          self.RESIZED_SCREEN_X, self.RESIZED_SCREEN_Y),
                                                    dtype=cupy.float32))
    ...
    def _choose_next_action(self, state):
        new_action = cupy.zeros([self.ACTIONS_COUNT])
    ...
    def _train(self):
        ...
        previous_states = cupy.ndarray(shape=(self.MINI_BATCH_SIZE, 4, self.RESIZED_SCREEN_X, self.RESIZED_SCREEN_Y),
                                       dtype=cupy.float32)
        actions = cupy.ndarray(shape=(self.MINI_BATCH_SIZE, 4), dtype=cupy.float32)
        rewards = cupy.ndarray(shape=(self.MINI_BATCH_SIZE, 1), dtype=cupy.float32)
        current_states = cupy.ndarray(shape=(self.MINI_BATCH_SIZE, 4, self.RESIZED_SCREEN_X, self.RESIZED_SCREEN_Y),
                                      dtype=cupy.float32)
        # get the batch variables
        for i in xrange(self.MINI_BATCH_SIZE):
            previous_states[i] = cupy.asarray(mini_batch[i][0])
            actions[i] = cupy.asarray(mini_batch[i][1])
            rewards[i] = cupy.asarray(mini_batch[i][2])
            current_states[i] = cupy.asarray(mini_batch[i][3])
        ...
        agents_target_q_reward_per_action = np.asanyarray(tmp, dtype=cupy.float32)
        ...
        if self._time % self.SAVE_EVERY_X_STEPS == 0:
            print "model saved"
            serializers.save_npz(self._checkpoint_path + "/model" + str(self._time), self.chainer_dqn_class.model)
