    ...
    def __init__(self, checkpoint_path="deep_q_pong_networks", playback_mode=False, verbose_logging=False):
        ...
        with tf.device('/gpu:0'):
            ...
            with tf.variable_scope('softmax_linear'):
                ...
                self.output_advantage = tf.add(tf.matmul(fiction_dropout, self.weights6), self.biases6)

            # local4
            with tf.variable_scope('local4') as scope:
                self.weights5 = _variable_with_weight_decay('weights', shape=[256, 256],
                                                            stddev=0.01, wd=0.0)
                self.biases5 = _variable_on_cpu('biases', [256], tf.constant_initializer(0.0))
                local4 = tf.nn.relu(tf.matmul(reshape, self.weights5) + self.biases5, name=scope.name)
                # _activation_summary(local4)

            with tf.variable_scope('softmax_linear2'):
                self.weights7 = _variable_with_weight_decay('weights', [256, 1],
                                                            stddev=0.01, wd=0.0)
                self.biases7 = _variable_on_cpu('biases', [1],
                                                tf.constant_initializer(0.01))
                self.output_value = tf.add(tf.matmul(local4, self.weights7), self.biases7)

            output_advantage_average = tf.reduce_mean(self.output_advantage)
            self.output_layer = tf.add(self.output_advantage, self.output_value) - output_advantage_average
        ...
        with tf.device('/gpu:0'):
            ...
            target_reshape = tf.reshape(target_pool3, [-1, 256])
            with tf.variable_scope('target_local3'):
                ...
                target_local3 = tf.nn.relu(tf.matmul(target_reshape, self.target_weights4) + self.target_biases4)
            ...
            with tf.variable_scope('target_softmax_linear'):
                ...
                self.target_output_advantage = tf.add(tf.matmul(target_fiction_dropout, self.target_weights6),
                                                      self.target_biases6)

            # local4
            with tf.variable_scope('target_local4') as scope:
                self.target_weights5 = _variable_with_weight_decay('weights', shape=[256, 256],
                                                                   stddev=0.01, wd=0.0)
                self.target_biases5 = _variable_on_cpu('biases', [256], tf.constant_initializer(0.0))
                target_local4 = tf.nn.relu(tf.matmul(target_reshape, self.target_weights5)
                                           + self.target_biases5, name=scope.name)

            with tf.variable_scope('target_softmax_linear2'):
                self.target_weights7 = _variable_with_weight_decay('weights', [256, 1],
                                                                   stddev=0.01, wd=0.0)
                self.target_biases7 = _variable_on_cpu('biases', [1],
                                                       tf.constant_initializer(0.01))
                self.target_output_value = tf.add(tf.matmul(target_local4, self.target_weights7), self.target_biases7)

            target_output_advantage_average = tf.reduce_mean(self.target_output_advantage)
            self.target_output_layer = tf.add(self.target_output_advantage,
                                              self.target_output_value) - target_output_advantage_average
