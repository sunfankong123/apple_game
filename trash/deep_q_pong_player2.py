...
class DeepQPongPlayer(PongPlayer):
    ...
    TARGET_NETWORK_UPDATE_FREQ = 1000  # target update frequency
    ...
    def _train(self):
        ...
        agents_reward_per_action = self._session.run(self._target_output_layer,
                                                     feed_dict={self._target_input_layer: current_states})
        ...
        if self._time % DeepQPongPlayer.TARGET_NETWORK_UPDATE_FREQ == 0 and self._time > 1:
            self._create_network(self._time)
    ...
    def _create_network(self, time):
        ...
        target_convolution_weights_1 = tf.Variable(
            tf.truncated_normal([8, 8, DeepQPongPlayer.STATE_FRAMES, 32], stddev=0.01))
        target_convolution_bias_1 = tf.Variable(tf.constant(0.01, shape=[32]))

        target_convolution_weights_2 = tf.Variable(tf.truncated_normal([4, 4, 32, 64], stddev=0.01))
        target_convolution_bias_2 = tf.Variable(tf.constant(0.01, shape=[64]))

        target_convolution_weights_3 = tf.Variable(tf.truncated_normal([3, 3, 64, 64], stddev=0.01))
        target_convolution_bias_3 = tf.Variable(tf.constant(0.01, shape=[64]))

        target_feed_forward_weights_1 = tf.Variable(tf.truncated_normal([256, 256], stddev=0.01))
        target_feed_forward_bias_1 = tf.Variable(tf.constant(0.01, shape=[256]))

        target_feed_forward_weights_2 = tf.Variable(
            tf.truncated_normal([256, DeepQPongPlayer.ACTIONS_COUNT], stddev=0.01))
        target_feed_forward_bias_2 = tf.Variable(tf.constant(0.01, shape=[DeepQPongPlayer.ACTIONS_COUNT]))
        ...
        # network weights
        target_input_layer = tf.placeholder("float",
                                            [None, DeepQPongPlayer.RESIZED_SCREEN_X, DeepQPongPlayer.RESIZED_SCREEN_Y,
                                             DeepQPongPlayer.STATE_FRAMES])

        target_hidden_convolutional_layer_1 = tf.nn.relu(
            tf.nn.conv2d(target_input_layer, target_convolution_weights_1, strides=[1, 4, 4, 1],
                         padding="SAME") + target_convolution_bias_1)
        ...
        target_hidden_convolutional_layer_2 = tf.nn.relu(
            tf.nn.conv2d(target_hidden_max_pooling_layer_1, target_convolution_weights_2, strides=[1, 2, 2, 1],
                         padding="SAME") + target_convolution_bias_2)
        ...
        target_hidden_convolutional_layer_3 = tf.nn.relu(
            tf.nn.conv2d(target_hidden_max_pooling_layer_2, target_convolution_weights_3,
                         strides=[1, 1, 1, 1], padding="SAME") + target_convolution_bias_3)
        ...
        target_final_hidden_activations = tf.nn.relu(
            tf.matmul(target_hidden_convolutional_layer_3_flat,
                      target_feed_forward_weights_1) + target_feed_forward_bias_1)

        target_output_layer = tf.matmul(target_final_hidden_activations,
                                        target_feed_forward_weights_2) + target_feed_forward_bias_2
        ...
        if time % DeepQPongPlayer.TARGET_NETWORK_UPDATE_FREQ == 0 and time > 1:

            print self._session.run(target_convolution_weights_1)
            change1 = target_convolution_weights_1.assign(convolution_weights_1)
            print self._session.run(target_convolution_weights_1)
            change2 = target_convolution_weights_2.assign(convolution_weights_2)
            change3 = target_convolution_weights_3.assign(convolution_weights_3)
            change4 = target_convolution_bias_1.assign(convolution_bias_1)
            change5 = target_convolution_bias_2.assign(convolution_bias_2)
            change6 = target_convolution_bias_3.assign(convolution_bias_3)
            change7 = target_feed_forward_weights_1.assign(feed_forward_weights_1)
            change8 = target_feed_forward_weights_2.assign(feed_forward_weights_2)
            change9 = target_feed_forward_bias_1.assign(feed_forward_bias_1)
            change10 = target_feed_forward_bias_2.assign(feed_forward_bias_2)

            self._session.run(fetches=[change1, change2, change3, change4, change5, change6, change7, change8, change9, change10])

        return input_layer, output_layer, target_input_layer, target_output_layer
