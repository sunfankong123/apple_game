...
def main():
    ...
    while len(holes)>0 and bar_score < FINISH_SCORE and keynum < 2000:
        clock.tick(60)
        screen.fill((0,0,0))
        all.update()
        all.draw(screen)
        ...
        paddle.update()
    stepsfile = open("stepsdata_r/stepsdata6200000.txt", "a")
...
class Paddle(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        elif stock == 4 or stock == 1 or stock == 2 or stock == 3:
            bricks_collided = pygame.sprite.spritecollide(self, self.bricks,True)
            if bricks_collided:
                stock += 1
                bar_score = 1
