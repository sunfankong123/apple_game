...
class DeepQPongPlayer(PongPlayer):
    ...
    INITIAL_RANDOM_ACTION_PROB = 0.7  # starting chance of an action being random
    ...
    def __init__(self, checkpoint_path="deep_q_pong_networks", playback_mode=False, verbose_logging=False):
        ...
        with tf.name_scope("loss"):
            ...
            tf.scalar_summary("loss(TargetQ-Q)", cost)
        self._train_operation = tf.train.AdamOptimizer(self.LEARN_RATE).minimize(cost)

        self.merged = tf.merge_all_summaries()
        self.writer = tf.train.SummaryWriter("pong_logs", self._session.graph)
    ...
    def _train(self):
        ...
        the_result, _ = self._session.run([self.merged, self._train_operation], feed_dict={
            self._input_layer: previous_states,
            self._action: actions,
            self._target: agents_expected_reward})

        summary_str = the_result[0]
