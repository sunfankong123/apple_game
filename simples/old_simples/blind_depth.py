...
def main():
    ...
    """
    # パドル
    paddle = Paddle(bricks, holes, angle, all, screen)

    clock = pygame.time.Clock()

    global perepisode, terminal_key, times, last_time, bar_score, display
    terminal_key = False
    ...
    while len(holes) > 0 and keynum < perepisode and final == False:
        ...
        if display % 10 == 0:
            pygame.display.update()
            # print pygame.surfarray.array2d(pygame.display.get_surface())
        print pygame.surfarray.array2d(pygame.display.get_surface())
    terminal_key = True
...
class Paddle(pygame.sprite.Sprite):
    """ボールを打つパドル"""
    def __init__(self, bricks, holes, angle, all, screen):
        # containersはmain()でセットされる
        pygame.sprite.Sprite.__init__(self, self.containers)
        # self.image, self.rect = load_image("paddle4.png")
        # colorkey = self.image.get_at((0, 2))
        # self.image.set_colorkey(colorkey, RLEACCEL)
        self.image = pygame.Surface((1, 1), 0, 32)
        self.rect = self.image.fill((255, 122, 122))
    ...
    def update(self):
        ...
        for event in pygame.event.get():  # User did something
            if event.type == KEYDOWN:
                # print pygame.surfarray.array3d(self.screen)
                # print bar_score
                bar_score = 0
                keynum += 1
                times += 1
            ...
            for brick in self.bricks:
                old_x = brick.rect.left / 1
                old_y = brick.rect.top / 1
                pygame.sprite.Group.remove(self.bricks, brick)
                pygame.sprite.Group.remove(self.alls, brick)
                a = pygame.sprite.spritecollide(brick, self.bricks, False)
            ...
            for hole in self.holes:
                old_x = hole.rect.left / 1
                old_y = hole.rect.top / 1
                pygame.sprite.Group.remove(self.holes, hole)
                pygame.sprite.Group.remove(self.alls, hole)
