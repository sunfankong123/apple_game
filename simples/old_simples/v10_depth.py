...
class Brick(pygame.sprite.Sprite):
    def __init__(self, x, y, level):
        ...
        self.level = level
        self.depth = 87 + (self.level + 1) * 42
        if self.depth > 255:
            self.depth = 255
            bar_score -= 10
            terminal_key = True
        self.image = pygame.Surface((16, 16), 0, 32)
        self.rect = self.image.fill((self.depth, 123, self.depth))
        # ブロックの位置を更新
...
class Hole(pygame.sprite.Sprite):
    def __init__(self, x, y, level):
        ...
        self.rect = self.image.fill((123, self.depth, self.depth))
        # ブロックの位置を更新
        self.rect.left = SCR_RECT.left + x * self.rect.width
        self.rect.top = SCR_RECT.top + y * self.rect.height
